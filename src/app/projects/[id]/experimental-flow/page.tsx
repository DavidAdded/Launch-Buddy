...
import { ExperimentalFlowView } from "./experimental-flow-view";

export default async function ExperimentalFlowPage({
  params,
}: {
  ...
  const { data: project } = await supabase
    .from("projects")
    .select("id, name, prod_url, customers(name)")
    .eq("id", id)
    .single();
  ...
  const { data: requests } = await supabase
    ...
    .select("id, model_name, company_name, status, error_message, parsed_response, raw_response, created_at")
    .eq("project_id", id)
